...
const BattleScene = (props) => {
  ...
  useEffect(() => {
    ...
    // Add our perspective camera
    const camera = new THREE.PerspectiveCamera( 45, window.innerWidth / window.innerHeight, 1, 1000 );
    camera.updateProjectionMatrix();

    // We don't want the canvas background so let's set this alpha to true
    const renderer = new THREE.WebGLRenderer({ alpha: true });

    // Set lights (2 for now, may be one later on)
    ...
    // Set camera position based on if menu is open or not; user is still free to move around with orbit controls
    // but will reset on each menu open. TODO: toggle button for this
    if (props.cameraPan.camera) {
